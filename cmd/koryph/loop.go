...
func cmdLoop(args []string, stdout, stderr io.Writer) int {
	...
	supersedeFailedCanaryCohort := fs.Bool(
		"supersede-failed-canary-cohort", false,
		"explicitly archive a failed prior generation while replacing its fixed cohort",
	)
	...
	setGroupedUsage(fs, stdout,
		...
		[]flagGroup{
			{title: "SCOPE", names: []string{"project", "parent", "max", "canary-cohort", "supersede-failed-canary-policy", "supersede-failed-canary-cohort", "dispatch-mode"}},
	...
	if *supersedeFailedCanaryCohort && len(canaryIDs) == 0 {
		return usageErr(stderr, "loop: --supersede-failed-canary-cohort requires --canary-cohort")
	}
	...
	if len(canaryIDs) > 0 && state.Canary != nil &&
		...
		var finalized bool
		state, finalized, err = finalizeStoppedNativeCanaryIntervention(
			ctx, rec.Root, rec.ProjectID, currentSpec,
		)
		if err != nil {
			return fail(stderr, fmt.Errorf("loop: finalize interrupted canary generation: %w", err))
		}
		if finalized {
			fmt.Fprintf(stdout, "loop %s: published failed evidence for contained interrupted canary\n", rec.ProjectID)
		}
		var rolled bool
	...
	if len(canaryIDs) > 0 && state.Canary != nil &&
		...
		state, retired, err = retireStaleNativeCanaryEvidence(
			...
			nativeCanaryRetireOptions{
				...
				SupersedeCohort: *supersedeFailedCanaryCohort,
			},
		)
		if err != nil {
...
func retireStaleNativeCanaryEvidence(
	...
	cohortMatches := sameNativeCanaryCohortMembership(canary, current)
	fixedEnvelopeMatches := sameNativeCanaryFixedEnvelope(canary, current)
	policyMatches := sameNativeCanaryPolicyInputs(canary, current)
	if !fixedEnvelopeMatches {
		return state, false, errors.New("canary width, inactivity, or report-path drift cannot be superseded")
	}
	if !cohortMatches && !option.SupersedeCohort {
		return state, false, errors.New("canary cohort drift requires explicit --supersede-failed-canary-cohort")
	}
	if !policyMatches && !option.SupersedePolicy {
		return state, false, errors.New("canary contract or execution policy drift requires explicit --supersede-failed-canary-policy")
	}
	...
	if (!cohortMatches || !policyMatches) && !binaryAdvanced {
		return state, false, errors.New("canary supersession requires a clean strict-descendant installed binary")
	}
	...
	history := nativeCanaryHistoryRecord{
		...
		Reason: nativeCanaryRolloverReason(
			registryChanged, binaryAdvanced, !policyMatches, !cohortMatches,
		),
		ProjectID:                      projectID,
	...
	if !cohortMatches {
		previousCohort := append([]string(nil), canary.Cohort...)
		currentCohort := append([]string(nil), current.Cohort...)
		sort.Strings(previousCohort)
		sort.Strings(currentCohort)
		history.CohortSupersession = &loopsupervisor.CanaryCohortSupersession{
			OperatorRequested:    true,
			PreviousCohort:       previousCohort,
			CurrentCohort:        currentCohort,
			PreviousCohortDigest: canary.CohortDigest,
			CurrentCohortDigest:  cohortDigestForNativeCanary(currentCohort),
		}
	}
	if markerExists {
	...
	if state.Canary != nil {
		...
		if err := store.ClearControl(); err != nil {
			return state, false, fmt.Errorf("retire stale canary control: %w", err)
		}
...
func finalizeStoppedNativeCanaryIntervention(
	ctx context.Context,
	root string,
	projectID string,
	current *loopsupervisor.CanarySpec,
	...
	canary := state.Canary
	if canary == nil || canary.Decision != "" || len(canary.RunIDs) == 0 {
		return state, false, nil
	}
	...
	currentGeneration, err := loopsupervisor.CanaryGenerationDigest(projectID, *current)
	...
	if state.Mode != loopsupervisor.ModeStopped || procx.Alive(state.PID) {
		return state, false, errors.New("interrupted canary supervisor is not stopped")
	}
	...
	if !control.Stop && !control.Drain {
		return state, false, errors.New("interrupted non-empty canary requires durable operator stop or drain intent")
	}
	...
	if canary.InstalledCommit == current.InstalledCommit ||
		canary.BuildIdentity == current.BuildIdentity {
		return state, false, errors.New("interrupted canary recovery requires a strict-descendant installed binary")
	}
	if err := authenticateNativeCanaryCheckout(root, current.InstalledCommit); err != nil {
		return state, false, fmt.Errorf("authenticate interrupted canary recovery checkout: %w", err)
	}
	if err := authenticateStrictDescendantCommit(
		root, canary.InstalledCommit, current.InstalledCommit,
	); err != nil {
	...
	ledgerStore := ledger.NewStore(root)
	aborted := false
	for _, runID := range canary.RunIDs {
		run, err := ledgerStore.LoadRun(runID)
		if err != nil {
			return state, false, fmt.Errorf("load interrupted canary run %s: %w", runID, err)
		}
		switch run.Status {
		case ledger.RunAborted:
			aborted = true
		case ledger.RunDone, ledger.RunDrained:
		default:
			return state, false, fmt.Errorf("interrupted canary run %s is not terminal", runID)
		}
		for phaseID, slot := range run.Slots {
			if slot == nil || !ledger.Terminal(slot.Status) {
				return state, false, fmt.Errorf("interrupted canary slot %s/%s is not terminal", runID, phaseID)
			}
			if slot.PID > 0 && procx.Alive(slot.PID) {
				return state, false, fmt.Errorf("interrupted canary slot %s/%s still has a live process", runID, phaseID)
			}
		}
	}
	if !aborted {
		return state, false, errors.New("interrupted canary has no contained aborted run")
	}

	if canary.HardStop == "" {
		canary.HardStop = "operator-intervention"
	}
	if canary.HardStop != "operator-intervention" {
		return state, false, errors.New("interrupted canary has a different hard-stop reason")
	}
	if canary.EndedAt == "" {
		canary.EndedAt = time.Now().UTC().Format(time.RFC3339Nano)
	}
	state.Mode = loopsupervisor.ModeCircuitOpen
	state.CircuitReason = "canary hard stop: operator-intervention"
	if err := store.SaveState(state); err != nil {
		return state, false, fmt.Errorf("checkpoint interrupted canary finalization: %w", err)
	}
	publication, err := (loopsupervisor.NativeCanaryPublisher{
		RepoRoot: root,
		Ledger:   ledgerStore,
	}).Publish(ctx, loopsupervisor.CanaryPublicationRequest{
		ProjectID: projectID,
		StartedAt: canary.StartedAt,
		EndedAt:   canary.EndedAt,
		Canary:    *canary,
	})
	if err != nil {
		return state, false, fmt.Errorf("publish interrupted canary evidence: %w", err)
	}
	if publication.Decision != "failed" {
		return state, false, errors.New("interrupted canary publisher returned a non-failed decision")
	}
	canary.Decision = publication.Decision
	canary.ReportDigest = publication.Digest
	canary.ReportGeneratedAt = publication.GeneratedAt
	canary.ReportPath = publication.Path
	canary.PublishedAt = time.Now().UTC().Format(time.RFC3339Nano)
	if err := store.SaveState(state); err != nil {
		return state, false, fmt.Errorf("checkpoint interrupted canary decision: %w", err)
	}
	return state, true, nil
}
...
func sameNativeCanaryNonBinaryPolicy(
	...
	return sameNativeCanaryCohortMembership(previous, current) &&
		sameNativeCanaryFixedEnvelope(previous, current) &&
		sameNativeCanaryPolicyInputs(previous, current)
}

func sameNativeCanaryPolicyInputs(
	previous *loopsupervisor.CanaryState,
	current *loopsupervisor.CanarySpec,
) bool {
...
func sameNativeCanaryCohortMembership(
	previous *loopsupervisor.CanaryState,
	current *loopsupervisor.CanarySpec,
) bool {
	if previous == nil || current == nil {
	...
	return slices.Equal(previousCohort, currentCohort) &&
		previous.CohortDigest == cohortDigestForNativeCanary(previousCohort)
}

func sameNativeCanaryFixedEnvelope(
	previous *loopsupervisor.CanaryState,
	current *loopsupervisor.CanarySpec,
) bool {
	if previous == nil || current == nil {
	...
	return previous.TargetWidth == current.TargetWidth &&
...
type nativeCanaryRetireOptions struct {
	...
	SupersedeCohort bool
}

func nativeCanaryRolloverReason(
	registryChanged, binaryAdvanced, policySuperseded, cohortSuperseded bool,
) string {
	switch {
	case policySuperseded && cohortSuperseded && registryChanged:
		return "installed binary and registry validation identity changed; operator superseded failed canary policy and cohort"
	case policySuperseded && cohortSuperseded:
		return "installed binary advanced; operator superseded failed canary policy and cohort"
	case cohortSuperseded && registryChanged:
		return "installed binary and registry validation identity changed; operator superseded failed canary cohort"
	case cohortSuperseded:
		return "installed binary advanced; operator superseded failed canary cohort"

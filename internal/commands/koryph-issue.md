...
5. Run `bd create` with:
   ...
   - engine changes are ordinary schedulable work against the installed binary; give them honest footprint labels and dependencies,
   - protected governance-path changes must be split into an operator-owned `HUMAN:` bead with `--label no-dispatch` rather than making an otherwise schedulable engine bead permanent backlog.
